"""200-sample PrimeVul stratified evaluation — V4 pipeline with confirm_it.

Writes full results to reports/eval_200_primevul_YYYY-MM-DD_HH-MM.csv
"""
import csv
import os
import sys
...
def main():
    ...
    # Load PrimeVul — stratified: first 100 vuln + 100 safe
    ...
    with open("data/primevul_test.csv", encoding="utf-8", errors="replace") as f:
        ...
        for row in reader:
            ...
            if target == 1 and len(vuln_samples) < 100:
                vuln_samples.append(s)
            elif target == 0 and len(safe_samples) < 100:
                safe_samples.append(s)
            if len(vuln_samples) >= 100 and len(safe_samples) >= 100:
                break

    samples = vuln_samples + safe_samples
    print(f"Loaded: {len(vuln_samples)} vuln + {len(safe_samples)} safe = {len(samples)} total")
    ...
    for i, s in enumerate(samples):
        ...
        if (i + 1) % 20 == 0:
            ...
            print(f"  [{i+1:3d}/{len(samples)}] {elapsed_total:.0f}s elapsed, ETA {eta:.0f}s")
    ...
    print(f"\n{'='*60}")
    print(f"Results: {len(valid)} valid + {skips} skipped, {elapsed:.0f}s, {total_calls} LLM calls")
    ...
    csv_path = f"reports/eval_200_primevul_{ts}.csv"
