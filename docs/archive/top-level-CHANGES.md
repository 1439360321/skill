...
> 实验日期：2026-07-12 | 模型：GLM-4.6V | 评测数据集：自建集（59样本）/ BigVul独立集（25样本，去重后）
...
| BigVul | 纯静态分析 | 0.370 | 0.625 | 0.263 | 0.500 | 5/3/14/3 |
| BigVul | LLM-First | **0.800** | 0.875 | 0.737 | 0.333 | 14/2/5/3 |
...
自建集消融出现A→B→C F1递减（0.91→0.78），原因是自建集的静态规则经过针对性调优，LLM引入反而增加误报。

### 消融实验（BigVul独立集，glm-4.6v）★关键

| 配置 | F1 | Precision | Recall | TP/FP/FN | LLM调用 |
|------|----|-----------|--------|----------|:------:|
| A: 纯静态基线 | 0.370 | 0.625 | 0.263 | 5/3/14 | 0 |
| B: + CWE专项Prompt + 完整代码 | 0.765 | 0.867 | 0.684 | 13/2/6 | 21 |
| C: + 多温度投票验证 | **0.800** | 0.875 | 0.737 | 14/2/5 | 84 |

**BigVul消融揭示真实贡献**：静态分析在不针对测试集调优的情况下F1仅有0.37。CWE专项Prompt将F1提升至0.76（+0.39），多温度投票进一步提至0.80（+0.04）。LLM管道的价值在独立评测集上得到证实。

自建集与BigVul消融结论相反的根本原因：自建集的静态规则（sink_registry、sanitizer正则、risk判定）全部基于59条手写样本调优，F1=0.91属于过拟合后的"天花板"；BigVul上静态真实水平是0.37，LLM管道贡献了0.43的F1增量。
|------|----|-----------|--------|----------|
| A: 纯静态基线 | 0.906 | 0.906 | 0.906 | 29/3/3 |
| B: + CWE专项Prompt + 完整代码 | 0.707 | 0.580 | 0.906 | 29/21/3 |
| C: + 多温度投票验证 | 0.784 | 0.691 | 0.906 | 29/13/3 |
